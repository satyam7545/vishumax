...
const mapCMSPayloadToSiteData = (cms: CMSPublicPayload): SiteDataState => {
  ...
  // Update document title, SEO meta, and favicon dynamically
  if (s?.seoTitle) {
  ...
  return {
    navbar: {
      ...
      brandLogoImage: s?.brandLogoImage || '',
    ...
    about: {
      ...
      portraitImage: s?.aboutPortraitImage || '',
    ...
    contact: {
      telegramUrl: s?.socialTelegram
        ? (s.socialTelegram.startsWith('http')
            ? s.socialTelegram
...
export const SiteDataProvider: React.FC<{ children: React.ReactNode }> = ({ children }) => {
  // Initialize state immediately from localStorage cache if available, preventing any flash of dummy content
  const [siteData, setSiteData] = useState<SiteDataState>(() => {
    try {
      const cached = localStorage.getItem(CMS_CACHE_KEY);
      if (cached) {
        const parsed = JSON.parse(cached);
        return mapCMSPayloadToSiteData(parsed);
      }
    } catch {
      // ignore
    }
  ...
  // 1. Fetch live CMS data from SQLite database and update cache
    ...
    try {
      ...
      if (res.ok) {
        ...
        if (json.success && json.data) {
          try {
            localStorage.setItem(CMS_CACHE_KEY, JSON.stringify(json.data));
          } catch {
